...
import com.taixin.ArrayUtil;


/**
 * 零相位滤波函数
 * 
 * @Description 调用static方法：doFiltfilt执行滤波 20160618
 * 
 */
...
public class Filtfilt {
	 ...
	 */
	public static double[] doFiltfilt(ArrayList<Double> B, ArrayList<Double> A, double[] X) {
		...
		double[] Y = subvector_reverse(signal1, signal1.size() - nfact - 1, nfact);
		return Y;
	...
	public static void filter(ArrayList<Double> B, ArrayList<Double> A, ArrayList<Double> X, ArrayList<Double> Y,
		...
		for (int i = 0; i < input_size; i++) {
			...
			while (order != 0) {
				if (i >= order)
					Zi.set(order - 1,
							B.get(order) * X.get(i - order) - A.get(order) * Y.get(i - order) + Zi.get(order));
				--order;
			}
	...
	/*
	 * static void changeArray2(ArrayList<Double> vec, double a0) { for (int i =
	 * 0; i < vec.size(); i++) { vec.set(i, a0-vec.get(i)); } }
	 */
	 ...
	 * { for (Double doubleitem : tail) { vec.add(doubleitem); } }
	 */
	static void append_vector(ArrayList<Double> vec, double[] tail) {
		for (int i = 0; i < tail.length; i++) {
			vec.add(tail[i]);
		}
	}

	static double[] subvector_reverse(ArrayList<Double> vec, int idx_end, int idx_start) {
		double[] resultArray = new double[idx_end - idx_start + 1];
		ArrayUtil.arraySet(resultArray, 0, resultArray.length);

		int endindex = idx_end - idx_start;
		for (int i = idx_start; i <= idx_end; i++) {
			resultArray[endindex--] = vec.get(i);
		}
